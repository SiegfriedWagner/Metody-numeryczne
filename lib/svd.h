...
#ifndef NUMERYCZNE_SVD_H
#define NUMERYCZNE_SVD_H
#include <math.h>
#include <float.h>
#include <assert.h>
#include "matrix_d.h"
    ...
    if (t_k == 0.0) {
        ...
        for (int j = 0; j < A_k.cols; ++j) {
            if (j <= k)
                ...
                d_k = ((double) sign(A_k.data[k][k+1])) / (2.0 * t_k * y.data[k+1]);
...
void calculate_J(matrix A, matrix P, matrix Q, matrix J) {
    ...
    array alfa = create_zero_array(cols);
...
matrix calculate_Y(matrix K, array eigenK) {
    ...
    for (int i = 0; i < A.cols; ++i) {
        ...
        //zero_matrix(U);
        zero_matrix(P);
        // zero_matrix(solution);
        printf("\nA_i(%d)\n", i);
        print_matrix(A);
        doolitleLUP(A, L, U, P);
        printf("\nU_i(%d)\n", i);
        print_matrix(U);
        solution.data[solution.rows - 1][0] = 1.0;
...
void SVD(matrix A, matrix U, matrix V, matrix Sigma) {
    ...
    for (int i = 0; i < diag_values.size; i++) {
    ...
    for (int i = 0; i < K.cols; ++i)
        szcz.data[i] = sqrt(fabs(eigenK.data[eigenK.size - 1 - i]));
    printf("\neigenX\n");
    print_array(szcz);
